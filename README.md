# mongodb-operator - a Demo Operator
...
```sh
make docker-build docker-push IMG=<some-registry>/mongodb-operator:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/mongodb-operator:tag
```
